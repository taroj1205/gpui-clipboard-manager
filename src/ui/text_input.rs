...
pub fn bind_text_input_keys(cx: &mut App) {
    cx.bind_keys([
        ...
        KeyBinding::new("ctrl-shift-backspace", WordBackspace, None),
        KeyBinding::new("ctrl-shift-delete", WordDelete, None),
        ...
        KeyBinding::new("ctrl-home", Home, None),
        KeyBinding::new("ctrl-end", End, None),
...
impl TextInput {
    ...
    fn word_right(&mut self, _: &WordRight, _: &mut Window, cx: &mut Context<Self>) {
        if self.selected_range.is_empty() {
            self.move_to(self.next_word_start(self.selected_range.end), cx);
        } else {
            self.move_to(self.selected_range.end, cx)
        }
    }
    ...
    fn previous_word_start(&self, offset: usize) -> usize {
        ...
        for (start, _) in content.unicode_word_indices() {
            if start < offset {
                last_start = start;
                found = true;
            } else {
        ...
        if found { last_start } else { 0 }
    }

    fn next_word_start(&self, offset: usize) -> usize {
        let content = &self.content;
        let offset = offset.min(content.len());
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color) = if content.is_empty() {
            (input.placeholder.clone(), hsla(255., 255., 255., 0.35))
        } else {
            (content, style.color)
        };
